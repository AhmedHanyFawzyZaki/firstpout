    ...
    */
    ...
    private function createImagesIfNotExists() 
    {
        if (!file_exists($this->_imageFile)) {
			...
			if(($image->width/$image->height)!=($this->_width/$this->_height)){
				if($image->width>$image->height){
					$image->resize($this->_width+10, $this->_height+10, 3);
				}else{
					$image->resize($this->_width+10, $this->_height+10, 4);
				}
			}
			$image->crop($this->_width+8, $this->_height+8);
