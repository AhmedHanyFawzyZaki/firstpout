...
class ApiController extends Controller {
                ...
                if (isset($request['userId']) && isset($request['offset']) && isset($request['limit'])) {
                    ...
                    if (!isset($request['babyId']) && !isset($request['groupId'])) {
                        if (isset($request['friends']) && $request['friends'] == 1) {
                            $posts = Post::model()->findAllBySql('select * from '.Post::model()->tableSchema->name.' where user_id=' . $request['userId'] . ' or 
							(
									user_id in (select user_id from '.UserFriend::model()->tableSchema->name.' where friend_id=' . $request['userId'] . '  and approved=1 and date_created <= `'.Post::model()->tableSchema->name.'`.date_created) or 
									user_id in (select friend_id from '.UserFriend::model()->tableSchema->name.' where user_id=' . $request['userId'] . ' and approved=1 and date_created <= `'.Post::model()->tableSchema->name.'`.date_created)
							) or 
							(
									group_id in (select id from `'.Group::model()->tableSchema->name.'` where user_id=' . $request['userId'] . ' or id in (select group_id from '.GroupUser::model()->tableSchema->name.' where user_id=' . $request['userId'] . ' and date_created <= `'.Post::model()->tableSchema->name.'`.date_created))
    ...
    public function actionUserFriends() {
        try {
            ...
            if ($request != false) {
                if ($request['id']) {
                    $users = User::model()->findAllBySql('select * from '.User::model()->tableSchema->name.' where id!=' . $request['id'] . ' and (id in (select user_id from '.UserFriend::model()->tableSchema->name.' where friend_id=' . $request['id'] . '  and approved=1) or id in (select friend_id from '.UserFriend::model()->tableSchema->name.' where user_id=' . $request['id'] . ' and approved=1))');
                    $arr = array();
                    if ($users) {
                        foreach ($users as $us) {
                            $arr[] = $this->fetchUserObject($us);
	...
	public function actionGetGroups() {
        try {
            $request = $this->parseRequest();
            if ($request != false) {
				if($request['name']=='no name')
                	$models = Group::model()->findAll();
				else
					$models = Group::model()->findAll(array('condition'=>'LOWER(title) like "%'.strtolower($request['name']).'%"'));
					
                if ($models) {
                    $response["message"] = $this->MESSAGE_SUCCESS;
					foreach($models as $model){
	                    $response["groups"][] = $this->fetchGroupObject($model);
					}
                }
                echo json_encode($response, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
            }
        ...
        try {
            ...
            if ($request != false) {
				if (Baby::IsBabyAccess($request['babyId'], $request['userId'])) {
					...
					$model->title=$request['title'];
					$model->date_of_visit=date('Y-m-d H:i:s', strtotime($request['date']));
					$model->medication=$request['medication'];
					$model->desage=$request['desage'];
					$model->bage_on=$request['bage_on'];
					$model->frequency=$request['frequency'];
					$model->note=$request['note'];
					$model->prescription=$request['prescription'];
					$model->diagonise=$request['diagonise'];
	...
	public function actionCreateDoctorHospital(){
		 try {
            $request = $this->parseRequest();
            if ($request != false) {
				$model = new User();
				$model->email = $request["email"];
				$model->username = $request["profileName"];
				$model->phone = $request["phone"];
				...
				$model->active = 1;
				$model->groups_id = $request['type'];  // 2=doctor , 3=hospital
				$model->image=$request['image'];
				if($model->image==''){
