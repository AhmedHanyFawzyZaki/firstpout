...
class ChatController extends AdminController {
    ...
    public function actionLoadMessages() {
        $id=$_REQUEST['id'];
        $messages = Chat::model()->findAll(array('condition' => 'admin=1 and (from_id=' . $id . ' OR to_id=' . $id . ')'));
        if($messages){
            $list='';
            foreach ($messages as $msg){
				$new='';
                if($msg->from_id==$id){
                    $class='pull-right leftBorder';
					if($msg->seen==0){
						$new='new';
						$msg->seen=1;
						$msg->save(false);
                    ...
                    $class='pull-left rightBorder';
				}
				$src=$msg->from->image?$msg->from->image:Yii::app()->request->baseUrl.'/img/user.jpg';
                $list.='<div class="row-fluid msg-cont '.$new.'">
                    <div class="span2 '.$class.'">
                        <img src="' . $src . '" width="50%" class="img-polaroid" style="margin: 18.5%">
                        <span class="msg-user">' . $msg->from->username.'</span>
                    </div>
                    <div class="span10">
                        '.$msg->msg.'
                    </div>
