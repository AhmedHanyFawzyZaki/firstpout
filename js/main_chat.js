    ...
    methods:
    {
      chat : function(){
        ...
        if( _trigger.length > 0 ){
          ...
          $('.friends-list > a').off('click').on('click', function(e){
            _self = $(this);
            e.preventDefault();
            _self.siblings('a').removeClass('current');
            dc.methods.openChatWindow(_self.attr('data-userid'), _self.position().top, _self.offset().top)
            _self.addClass('current');
          });

          $(document.body).on('click', '.close-window', function(e){
            e.preventDefault();
      ...
      openChatWindow: function(userID, triggedPos, offset){
          _chatWindow = $('#conversations'),
          _window = $(window),
          _userChat = $('#chat-userid-'+userID);
          _chatWindow.show();
          //$('#conversations').find('.chat-window').hide();
          if( _userChat.length > 0 ){
            _userChat.show();
          }/*else{
            $.ajax({
              url: 'ajax-chat-window.html',
              data: {userid: userID},
              dataType : 'html',
            }).done(function(data){
              _chatWindow.append(data);
              $('#chat-userid-'+userID).show();
              $('.conversation-wrap').jScrollPane();
            });
          ...
          _chatWindow.css({
            ...
            //$('.conversation-wrap').jScrollPane();
			$('.conversation-wrap').css('overflow-y','scroll');
          });
      },
